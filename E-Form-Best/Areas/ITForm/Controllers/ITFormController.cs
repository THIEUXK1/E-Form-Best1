    ...
    public class ITFormController : Controller
    {
        ...
        public IActionResult DangKyDon()
        {
            ...
            var danhSachLoaiDon = new List<LoaiDonIt>
            {
                ...
                new LoaiDonIt { Stt = 2, Ten = "Yêu cầu hỗ trợ IT",     Icon = "fa-life-ring",     Mau = "#ea580c", Url = "/FormIT/TaoIT_Order",         MoTa = "Hỗ trợ nhanh: sửa chữa, xử lý sự cố máy tính, máy in, mạng, phần mềm. Gửi thẳng cho IT, không cần chủ quản duyệt." },
                ...
                new LoaiDonIt { Stt = 10, Ten = "Cài đặt phần mềm",     Icon = "fa-download",      Mau = "#2563eb", Url = "/FormIT/DonCaiDatPhanMem",    MoTa = "Yêu cầu IT cài đặt phần mềm phục vụ công việc lên máy tính được chỉ định." },
                new LoaiDonIt { Stt = 11, Ten = "Lập trình ứng dụng",   Icon = "fa-code",          Mau = "#0f766e", Url = "/FormIT/DonLapTrinhUngDung",  MoTa = "Đặt IT viết mới hoặc nâng cấp web app, tool, macro Excel, script tự động hoá, dashboard." },
                new LoaiDonIt { Stt = 12, Ten = "Trả thiết bị",         Icon = "fa-undo",          Mau = "#dc2626", Url = "/FormIT/DonTraThietBi",       MoTa = "Trả lại IT thiết bị gặp trục trặc, hư hỏng hoặc không còn nhu cầu sử dụng." }
            };
        ...
        #region Don Cai Dat Phan Mem (Form IT 10)

        // Tên công việc trong bảng CongViecIT dùng để lọc nhân sự IT phụ trách cài phần mềm.
        private const string CongViecCaiDatPhanMem = "Cài đặt phần mềm";

        /// <summary>Nạp danh sách nhân sự IT đang đảm nhận công việc "Cài đặt phần mềm".</summary>
        private List<ItNguoiHoTro> LayNhanSuHoTroPhanMem()
        {
            return _context.ItNguoiHoTros
                .Include(x => x.CongViecIts)
                .Where(x => x.BoPhan == "IT")
                .Select(x => new ItNguoiHoTro
                {
                    ...
                    BoPhan = x.BoPhan,
                    GhiChu = x.GhiChu,
                    CongViecIts = x.CongViecIts.Where(cv => cv.Ten == CongViecCaiDatPhanMem).ToList()
                })
                .Where(x => x.CongViecIts.Any())
                .ToList();
        }

        [HttpGet("/FormIT/DonCaiDatPhanMem")]
        public IActionResult DonCaiDatPhanMem()
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Redirect("/DonXetDuyet/DangNhap");

            ViewBag.ListNguoiHoTro = LayNhanSuHoTroPhanMem();
        ...
        // Luồng AJAX: mọi nhánh đều trả JSON, view không bao giờ tải lại trang.
        [HttpPost("/FormIT/DonCaiDatPhanMem")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DonCaiDatPhanMem(FormIt form, [FromForm] ItCaiDatPhanMem10 phanMem, int[] SelectedCongViecIds)
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Json(new { thanhCong = false, thongBao = "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại rồi gửi đơn." });
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    form.TenCongTy = tenCongTy;
                    form.TrangThai = "ChoDuyet";
                    form.IdForm = "IT_CaiDatPhanMem_10";
                    form.TenForm = "Đơn yêu cầu cài đặt phần mềm";
                    form.Danhmuc = CongViecCaiDatPhanMem;
                    ...
                    {
                        string extension = Path.GetExtension(uploadFile.FileName);
                        string fileName = $"DonPhanMem_ID{form.Id}_{safeName}_{timeStamp}{extension}";
                        using (var fileStream = new FileStream(Path.Combine(networkPath, fileName), FileMode.Create))
                        {
                            await uploadFile.CopyToAsync(fileStream);
                        }
                    ...
                    // --- BƯỚC 3: CHI TIẾT PHẦN MỀM + ẢNH MINH HOẠ ---
                    if (phanMem != null)
                    {
                        phanMem.IdFormIt = form.Id;
                        ...
                        if (anhFile != null && anhFile.Length > 0)
                        {
                            ...
                            string imgFileName = $"AnhPhanMem_ID{form.Id}_{safeName}_{timeStamp}{imgExtension}";
                            using (var imgStream = new FileStream(Path.Combine(networkPath, imgFileName), FileMode.Create))
                            {
                                await anhFile.CopyToAsync(imgStream);
                            }

                            phanMem.DuonDanAnh = imgFileName;
                        }

                        _context.ItCaiDatPhanMem10s.Add(phanMem);
                        await _context.SaveChangesAsync();
                    }
                    ...
                    {
                        idNhanSuHoTro = await _context.CongViecIts
                            .Where(cv => cv.Ten == CongViecCaiDatPhanMem && cv.IdItNguoiHoTro != null)
                            .Select(cv => cv.IdItNguoiHoTro!.Value)
                            .Distinct()
                            .ToListAsync();
                    }
                    ...
                    // --- BƯỚC 5: LỊCH SỬ ---
                    string moTaChiTiet = $"[Khởi tạo đơn] Người tạo: {userName} (ID: {userId}) | Bộ phận: {phongBan} | Công ty: {tenCongTy}\n" +
                                         $"- Phần mềm: {phanMem?.TenPhanMem ?? "N/A"} (phiên bản: {(string.IsNullOrWhiteSpace(phanMem?.PhienBan) ? "không nêu" : phanMem.PhienBan)})\n" +
                                         $"- Máy cần cài: {phanMem?.MayCaiDat ?? "N/A"}\n" +
                                         $"- Ngày cần có: {(phanMem?.NgayCanCo?.ToString("dd/MM/yyyy") ?? "N/A")}\n" +
                                         $"- Lý do sử dụng: {phanMem?.LyDoSuDung ?? "N/A"}\n" +
                                         $"- Ảnh minh hoạ: {(string.IsNullOrEmpty(phanMem?.DuonDanAnh) ? "Không có" : phanMem.DuonDanAnh)}\n" +
                                         $"- File đính kèm: {(string.IsNullOrEmpty(form.FileDinhKem) ? "Không có" : form.FileDinhKem)}\n" +
                                         $"- Người hỗ trợ chỉ định: {danhSachTenHoTro}.";
                    ...
                    return Json(new
                    {
                        ...
                        trung = false,
                        idDon = form.Id,
                        thongBao = $"Đã gửi đơn yêu cầu cài đặt phần mềm (mã đơn {form.Id}). Đơn đang chờ duyệt."
                    });
                }
                catch (Exception ex)
                {
        ...
        #region Don Lap Trinh Ung Dung (Form IT 11)

        // Tên công việc trong bảng CongViecIT dùng để lọc nhân sự IT phụ trách lập trình.
        private const string CongViecLapTrinh = "Lập trình ứng dụng";

        /// <summary>Nạp danh sách nhân sự IT đang đảm nhận công việc "Lập trình ứng dụng".</summary>
        private List<ItNguoiHoTro> LayNhanSuHoTroLapTrinh()
        {
            return _context.ItNguoiHoTros
                .Include(x => x.CongViecIts)
                .Where(x => x.BoPhan == "IT")
                .Select(x => new ItNguoiHoTro
                {
                    ...
                    BoPhan = x.BoPhan,
                    GhiChu = x.GhiChu,
                    CongViecIts = x.CongViecIts.Where(cv => cv.Ten == CongViecLapTrinh).ToList()
                })
                .Where(x => x.CongViecIts.Any())
                .ToList();
        }

        [HttpGet("/FormIT/DonLapTrinhUngDung")]
        public IActionResult DonLapTrinhUngDung()
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Redirect("/DonXetDuyet/DangNhap");

            ViewBag.ListNguoiHoTro = LayNhanSuHoTroLapTrinh();
        ...
        // Luồng AJAX: mọi nhánh đều trả JSON, view không bao giờ tải lại trang.
        [HttpPost("/FormIT/DonLapTrinhUngDung")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DonLapTrinhUngDung(FormIt form, [FromForm] ItLapTrinhUngDung11 ungDung, int[] SelectedCongViecIds)
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Json(new { thanhCong = false, thongBao = "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại rồi gửi đơn." });
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    var donVuaTao = await _context.FormIts
                        .Where(x => x.IdNguoiTao == userId
                                    && x.IdForm == "IT_LapTrinhUngDung_11"
                                    && x.TimeNguoiTao != null
                                    && x.TimeNguoiTao > nguongTrung)
                        .OrderByDescending(x => x.TimeNguoiTao)
                        .Select(x => new { x.Id })
                    ...
                    form.TenCongTy = tenCongTy;
                    form.TrangThai = "ChoDuyet";
                    form.IdForm = "IT_LapTrinhUngDung_11";
                    form.TenForm = "Đơn yêu cầu lập trình ứng dụng";
                    form.Danhmuc = CongViecLapTrinh;
                    ...
                    {
                        string extension = Path.GetExtension(uploadFile.FileName);
                        string fileName = $"DonLapTrinh_ID{form.Id}_{safeName}_{timeStamp}{extension}";
                        using (var fileStream = new FileStream(Path.Combine(networkPath, fileName), FileMode.Create))
                        {
                            await uploadFile.CopyToAsync(fileStream);
                        }
                    ...
                    // --- BƯỚC 3: CHI TIẾT YÊU CẦU + ẢNH MINH HOẠ ---
                    if (ungDung != null)
                    {
                        ungDung.IdFormIt = form.Id;
                        ...
                        if (anhFile != null && anhFile.Length > 0)
                        {
                            ...
                            string imgFileName = $"AnhLapTrinh_ID{form.Id}_{safeName}_{timeStamp}{imgExtension}";
                            using (var imgStream = new FileStream(Path.Combine(networkPath, imgFileName), FileMode.Create))
                            {
                                await anhFile.CopyToAsync(imgStream);
                            }

                            ungDung.DuonDanAnh = imgFileName;
                        }

                        _context.ItLapTrinhUngDung11s.Add(ungDung);
                        await _context.SaveChangesAsync();
                    }
                    ...
                    {
                        idNhanSuHoTro = await _context.CongViecIts
                            .Where(cv => cv.Ten == CongViecLapTrinh && cv.IdItNguoiHoTro != null)
                            .Select(cv => cv.IdItNguoiHoTro!.Value)
                            .Distinct()
                            .ToListAsync();
                    }
                    ...
                    // --- BƯỚC 5: LỊCH SỬ ---
                    string moTaChiTiet = $"[Khởi tạo đơn] Người tạo: {userName} (ID: {userId}) | Bộ phận: {phongBan} | Công ty: {tenCongTy}\n" +
                                         $"- Loại yêu cầu: {ungDung?.LoaiYeuCau ?? "N/A"}\n" +
                                         $"- Tên ứng dụng: {ungDung?.TenUngDung ?? "N/A"}\n" +
                                         $"- Mức độ ưu tiên: {ungDung?.MucDoUuTien ?? "N/A"} | Ngày mong muốn: {(ungDung?.NgayMongMuon?.ToString("dd/MM/yyyy") ?? "N/A")}\n" +
                                         $"- Người sử dụng: {ungDung?.NguoiSuDung ?? "N/A"} | Bộ phận: {ungDung?.BoPhanSuDung ?? "N/A"}\n" +
                                         $"- Hệ thống liên quan: {ungDung?.HeThongLienQuan ?? "N/A"} | Tần suất: {ungDung?.TanSuatSuDung ?? "N/A"}\n" +
                                         $"- Mô tả yêu cầu: {ungDung?.MoTaYeuCau ?? "N/A"}\n" +
                                         $"- Ảnh minh hoạ: {(string.IsNullOrEmpty(ungDung?.DuonDanAnh) ? "Không có" : ungDung.DuonDanAnh)}\n" +
                                         $"- File đính kèm: {(string.IsNullOrEmpty(form.FileDinhKem) ? "Không có" : form.FileDinhKem)}\n" +
                                         $"- Người hỗ trợ chỉ định: {danhSachTenHoTro}.";
                    ...
                    return Json(new
                    {
                        ...
                        trung = false,
                        idDon = form.Id,
                        thongBao = $"Đã gửi đơn yêu cầu lập trình ứng dụng (mã đơn {form.Id}). Đơn đang chờ duyệt."
                    });
                }
                catch (Exception ex)
                {
        ...
        #region Don Tra Thiet Bi (Form IT 12)

        // Tên công việc trong bảng CongViecIT dùng để lọc nhân sự IT phụ trách nhận thiết bị trả về.
        private const string CongViecTraThietBi = "Trả thiết bị";

        /// <summary>Nạp danh sách nhân sự IT đang đảm nhận công việc "Trả thiết bị".</summary>
        private List<ItNguoiHoTro> LayNhanSuHoTroTraThietBi()
        {
            return _context.ItNguoiHoTros
                .Include(x => x.CongViecIts)
                .Where(x => x.BoPhan == "IT")
                .Select(x => new ItNguoiHoTro
                {
                    ...
                    BoPhan = x.BoPhan,
                    GhiChu = x.GhiChu,
                    CongViecIts = x.CongViecIts.Where(cv => cv.Ten == CongViecTraThietBi).ToList()
                })
                .Where(x => x.CongViecIts.Any())
                .ToList();
        }

        [HttpGet("/FormIT/DonTraThietBi")]
        public IActionResult DonTraThietBi()
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Redirect("/DonXetDuyet/DangNhap");

            ViewBag.ListNguoiHoTro = LayNhanSuHoTroTraThietBi();
        ...
        // Luồng AJAX: mọi nhánh đều trả JSON, view không bao giờ tải lại trang.
        [HttpPost("/FormIT/DonTraThietBi")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DonTraThietBi(FormIt form, [FromForm] ItTraThietBi12 thietBi)
        {
            if (User == null || User.Identity == null || !User.Identity.IsAuthenticated)
                return Json(new { thanhCong = false, thongBao = "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại rồi gửi đơn." });
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    form.TenCongTy = tenCongTy;
                    form.TrangThai = "ChoDuyet";
                    form.IdForm = "IT_TraThietBi_12";
                    form.TenForm = "Đơn trả thiết bị";
                    form.Danhmuc = CongViecTraThietBi;
                    ...
                    {
                        string extension = Path.GetExtension(uploadFile.FileName);
                        string fileName = $"DonTraTB_ID{form.Id}_{safeName}_{timeStamp}{extension}";
                        using (var fileStream = new FileStream(Path.Combine(networkPath, fileName), FileMode.Create))
                        {
                            await uploadFile.CopyToAsync(fileStream);
                        }
                    ...
                    // --- BƯỚC 3: CHI TIẾT THIẾT BỊ + ẢNH ---
                    if (thietBi != null)
                    {
                        thietBi.IdFormIt = form.Id;
                        ...
                        if (anhFile != null && anhFile.Length > 0)
                        {
                            ...
                            string imgFileName = $"AnhTraTB_ID{form.Id}_{safeName}_{timeStamp}{imgExtension}";
                            using (var imgStream = new FileStream(Path.Combine(networkPath, imgFileName), FileMode.Create))
                            {
                                await anhFile.CopyToAsync(imgStream);
                            }

                            thietBi.DuonDanAnh = imgFileName;
                        }

                        _context.ItTraThietBi12s.Add(thietBi);
                        await _context.SaveChangesAsync();
                    }

                    // --- BƯỚC 4: NGƯỜI HỖ TRỢ ---
                    // Khác các đơn còn lại: thiết bị trả về cần cả tổ IT cùng nắm, nên luôn gán
                    // TOÀN BỘ nhân sự đang đảm nhận "Trả thiết bị". Form không cho chọn từng người:
                    // chỉ có một ô tích xác nhận, nên không nhận SelectedCongViecIds từ client.
                    string danhSachTenHoTro = "Chưa chọn";
                    var idNhanSuHoTro = await _context.CongViecIts
                        .Where(cv => cv.Ten == CongViecTraThietBi && cv.IdItNguoiHoTro != null)
                        .Select(cv => cv.IdItNguoiHoTro!.Value)
                        .Distinct()
                        .ToListAsync();
                    ...
                    // --- BƯỚC 5: LỊCH SỬ ---
                    string moTaChiTiet = $"[Khởi tạo đơn] Người tạo: {userName} (ID: {userId}) | Bộ phận: {phongBan} | Công ty: {tenCongTy}\n" +
                                         $"- Loại thiết bị: {thietBi?.LoaiThietBi ?? "N/A"}\n" +
                                         $"- Tên thiết bị: {thietBi?.TenThietBi ?? "N/A"} | Serial/Mã tài sản: {thietBi?.SerialTaiSan ?? "N/A"}\n" +
                                         $"- Tình trạng: {thietBi?.TinhTrang ?? "N/A"}\n" +
                                         $"- Mô tả lỗi: {thietBi?.MoTaLoi ?? "N/A"}\n" +
                                         $"- Vị trí hiện tại: {thietBi?.ViTriHienTai ?? "N/A"} | Phụ kiện kèm theo: {thietBi?.PhuKienKemTheo ?? "Không có"}\n" +
                                         $"- Ngày trả: {(thietBi?.NgayTra?.ToString("dd/MM/yyyy") ?? "N/A")}\n" +
                                         $"- Ảnh thiết bị: {(string.IsNullOrEmpty(thietBi?.DuonDanAnh) ? "Không có" : thietBi.DuonDanAnh)}\n" +
                                         $"- File đính kèm: {(string.IsNullOrEmpty(form.FileDinhKem) ? "Không có" : form.FileDinhKem)}\n" +
                                         $"- Người hỗ trợ (gán toàn bộ tổ): {danhSachTenHoTro}.";
                    ...
                    return Json(new
                    {
                        ...
                        trung = false,
                        idDon = form.Id,
                        thongBao = $"Đã gửi đơn trả thiết bị (mã đơn {form.Id}). Đơn đang chờ duyệt."
                    });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> ChiTiet(int id)
        {
            ...
            var don = await _context.FormIts
                ...
                .Include(f => f.ItCaiDatPhanMem10s)
                .Include(f => f.ItLapTrinhUngDung11s)
                .Include(f => f.ItTraThietBi12s)
