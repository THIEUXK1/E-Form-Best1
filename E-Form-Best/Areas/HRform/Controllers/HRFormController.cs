    ...
    public class HRFormController : Controller
    {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    if (chiTiet != null)
                    {
                        ...
                        _context.CtHoTroTienDienThoai7s.Add(chiTiet);
                        await _context.SaveChangesAsync();
                    }
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    if (chiTiet != null)
                    {
                        ...
                        _context.CtDoiCaLam8s.Add(chiTiet);
                        await _context.SaveChangesAsync();
                    }
        ...
        public async Task<IActionResult> QuanLyXetDuyet()
        {
            ...
            else if (userRole == "ADMIN" || userRole == "QUANLY")
            {
                ...
                if (!string.IsNullOrEmpty(phongBan))
                {
                    ...
                    query = query.Where(f => f.PhanBo != null && f.PhanBo.Trim().ToLower() == phongBan.ToLower());
                }
                else
                {
        ...
        public async Task<IActionResult> HoanTatDon()
        {
            ...
            else if (userRole == "QUANLY")
            {
                ...
                if (!string.IsNullOrEmpty(phongBan))
                {
                    query = query.Where(f => f.PhanBo != null
                                          && f.PhanBo.Trim().ToLower() == phongBan.ToLower()
                                          && f.IdNguoiDuyet != null);
                }
                else
                {
        ...
        public async Task<IActionResult> DanhSachBaoVe()
        {
            ...
            */
            ...
            var model = await _context.FormHrs
                .Include(f => f.CtXinRaNgoai1s)           // Nạp chi tiết đơn xin ra ngoài
                .Include(f => f.CtMangHangHoaRaCong2s)    // Nạp chi tiết đơn mang hàng hóa
