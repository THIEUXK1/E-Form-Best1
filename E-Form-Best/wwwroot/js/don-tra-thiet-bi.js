/*
 * ĐƠN TRẢ THIẾT BỊ (IT_TraThietBi_12) — gửi đơn không tải lại trang.
...
(function () {
    ...
    var form = document.getElementById("traThietBiForm");
    if (!form) return;
    ...
    var oDanAnh = document.getElementById("paste_AnhThietBi");
    ...
    function kiemTraHopLe() {
        if (!form.querySelector('input[name="LoaiThietBi"]:checked'))
            return { thongBao: "Vui lòng chọn Loại thiết bị cần trả!" };

        var tenThietBi = document.getElementById("TenThietBi");
        if (!tenThietBi.value.trim())
            return { thongBao: "Vui lòng nhập Tên thiết bị!", idFocus: "TenThietBi" };

        var serial = document.getElementById("SerialTaiSan");
        if (!serial.value.trim())
            return { thongBao: "Vui lòng nhập Serial hoặc Mã tài sản — IT cần thông tin này để đối chiếu kiểm kê!", idFocus: "SerialTaiSan" };

        var moTa = document.getElementById("MoTaLoi");
        if (!moTa.value.trim())
            return { thongBao: "Vui lòng mô tả tình trạng trục trặc của thiết bị!", idFocus: "MoTaLoi" };

        var ngayTra = document.getElementById("NgayTra");
        if (!ngayTra.value)
            return { thongBao: "Vui lòng chọn Ngày trả thiết bị!", idFocus: "NgayTra" };

        // Ngày trả nằm trong quá khứ gần như luôn là gõ nhầm
        var homNay = new Date();
        homNay.setHours(0, 0, 0, 0);
        if (new Date(ngayTra.value) < homNay)
            return { thongBao: "Ngày trả không được nhỏ hơn ngày hôm nay!", idFocus: "NgayTra" };

        // Chỉ còn một ô tích: đơn luôn gửi cho cả tổ IT nên không chọn từng người
        var oToIT = document.getElementById("chkToIT");
        if (!oToIT || !oToIT.checked)
            return { thongBao: "Vui lòng tích xác nhận tổ IT tiếp nhận thiết bị!" };
    ...
    form.addEventListener("submit", async function (e) {
        ...
        if (window.Swal) {
            var xacNhan = await Swal.fire({
                icon: "question",
                title: "Xác nhận gửi đơn trả thiết bị?",
                text: document.getElementById("TenThietBi").value.trim() +
                      " — " + document.getElementById("SerialTaiSan").value.trim(),
                showCancelButton: true,
